...
{
	char delim_array[] = {'{', '}', '\"', ';', '#'};
	std::vector<char> delim(delim_array, delim_array + sizeof(delim_array) / sizeof(delim_array));
	std::string temp_str;
	...
	for (int i = 0; config_str[i]; i++)
	{
		...
		else
		{
			...
			{
				temp_str += config_str[i];
				std::cout << "[This is config_str[i]]: " << config_str[i] << std::endl;
				i++;
			}
			std::cout << "This is temp_str: " << temp_str << std::endl;
