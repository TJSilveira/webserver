...
Token& Parser::expect_and_advance(std::list<Token>::iterator &it, std::list<Token>::const_iterator &end, TokenType expected_type)
{
...
void Parser::parse_directive(std::vector<t_directive> &vector, std::list<Token>::iterator &it, std::list<Token>::const_iterator &end)
{
	t_directive temp_dir;

	temp_dir.name = expect_and_advance<InvalidLocation>(it, end, DirectTok).content;
...
void	Parser::parse_location(t_virtual_server &vir_serv, std::list<Token>::iterator &it, std::list<Token>::const_iterator &end)
{
	if (it->content.compare("location") != 0 || 
		it->type != ContextTok)
	{
	...
	temp_loc.path = expect_and_advance<InvalidLocation>(it, end, ParamTok).content;
...
void	Parser::parse_virtual_server(t_server &server, std::list<Token>::iterator &it, std::list<Token>::const_iterator &end)
{
	if (it->content.compare("server") != 0 || 
		it->type != ContextTok)
	{
...
void	Parser::parse_server(std::list<Token>::iterator &it, std::list<Token>::const_iterator &end)
{
	if (it->content.compare("http") != 0 || 
		it->type != ContextTok)
	{
		throw InvalidServer();		
	}
